...
// e.g. this should work great on project gutenberg EPUBs
...
program
    .requiredOption('-i, --input <file>', 'EPUB file')
    .option('-c, --config <file>', 'Config JSON file (see example)')
    .option('-f, --force', 'Overwrite existing output');
program.parse(process.argv);

let config = program.config ? JSON.parse(fs.readFileSync(path.resolve(__dirname, program.config)).toString()) : null;
...
async function main() {
    let tmpdir = await unzip(path.resolve(__dirname, program.input));
    ...
    console.log(navPointElms
    .filter(elm => config.skipNcxIds.includes(elm.getAttribute("id") == false)));
    ...
    readingOrder.map((item, idx) => {
      ...
      writeOut(outpath, itemDom.serialize());

    });

    let manifest = {
      readingOrder: readingOrder.map(item => ({name: item.name, url: item.url}))
    };
    
    writeOut(`${path.basename(program.input).replace('.epub', '')}.json`, JSON.stringify(manifest));
